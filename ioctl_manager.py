...
# FILES
IDENTITY_FILE = "my_personality.json"
WHITELIST_FILE = "whitelist.json"
SITES_FILE = "sites.json"

def GetOrCreateIdentity():
    if not os.path.exists(IDENTITY_FILE):
        print("{!!} my_personality.json not found. Creating")
        print("VIRTUAL_IP = 10.0.0.1/24. (Default IP. For setting, open file: my_personality.json)")
        ...
        print(f'Created your UUID: {my_uuid} and DEFAULT virtual ip: 10.0.0.1/24.')
...
def LoadWhiteList():
    ...
    if os.path.exists(WHITELIST_FILE):
        with open(WHITELIST_FILE, "r", encoding="utf-8") as f:
            print("D-line: opening file in READ only.")
            data = json.load(f)
...
def start():
    my_uuid = GetOrCreateIdentity()
    friends = LoadWhiteList()
    sites = load_sites()
    my_uuid, virtual_ip = GetOrCreateIdentity()
    tun = OpenInterface('cnvlp0', ip_addr=virtual_ip)
    ...
    with open(WHITELIST_FILE, "r", encoding="utf-8") as f:
        data = json.load(f)
        print(f"IpS in whitelist.json:", data['friends'][0]['ip'])
    print(":: Warn: standard port CnUlP: 63048.")
    print(f"!! + RX socket bound to 0.0.0.0:63048")
                ...
                if s is tun:
                    ...
                    if dst_ip in sites:
                        ...
                        if owner_uuid in friends:
                            friend_data = friends[owner_uuid]
                            real_desk_ip = friend_data["real_ip"]
                            ...
                            print(f"[>>] [0x24] Traffic for '{site_data['name']}' ({dst_ip}) -> Sent to {real_desk_ip}")
                        else:
                            print(f"[!!] Owner UUID {owner_uuid} NOT in whitelist!")
                    else:
                        print(f"[>>] [0x13] Unknown Destination: {dst_ip}")
                elif s is rx_socket:
                    try:
                    ...
                    if len(packet) > 5 and packet[0] == 0x24:
                        ...
                        if len(ip_packet) < 20 or (ip_packet[0] >> 4) != 4:
                            print(f"[!] Dropped from {addr}: invalid IP | Hex: {packet[:20].hex()}")
                            continue  
                        try:
                            tun.write(ip_packet)
                        except OSError as e:
